...
- **Never hand-roll a single-line `TextInput` — use `components/ui/input.tsx`.** Raw single-line TextInputs render their text/placeholder off-centre on iOS (regressed 10+ times), and a fixed-height box clips the text at larger Dynamic Type sizes — input box heights must be `min-h`, never `h`. Raw `TextInput` imports are lint-blocked (`no-restricted-imports`) outside the allowlist in `eslint.config.js`, and `pnpm check:inputs` (in `/verify`) catches fixed-height/named-text-size violations on the survivors. The primitive takes `leftIcon`/`rightIcon`, so search rows don't justify hand-rolling. Full rule: `.claude/rules/coding-conventions.md` → "Single-line text inputs".
