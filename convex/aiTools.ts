...
import { exerciseTypeValidator, ingredientValidator, macrosValidator, planDayStatusValidator } from "./validators";
...
/**
 * Creates workout templates from AI tool call output.
 * Called when a user approves a template or plan that includes templates.
 */
export const createTemplatesFromAI = internalMutation({
  args: {
    userId: v.id("users"),
    templates: v.array(
      v.object({
  ...
  handler: async (ctx, args) => {
    ...
    const createdTemplateClientIds: string[] = [];
    ...
    for (const template of args.templates) {
      ...
      const existingTemplate = await ctx.db
        ...
        .withIndex("by_user_clientId", (q) =>
          q.eq("userId", args.userId).eq("clientId", template.clientId)
        )
        .unique();

      if (existingTemplate) {
        createdTemplateClientIds.push(template.clientId);
        continue;
      }

      // Upsert exercises into the exercise library
      for (const ex of template.exercises) {
        const existingExercise = await ctx.db
          .query("exercises")
          .withIndex("by_user_clientId", (q) =>
            q.eq("userId", args.userId).eq("clientId", ex.exerciseClientId)
          )
          .unique();
        ...
        if (!existingExercise) {
          ...
          const byName = await ctx.db
            ...
            .withIndex("by_user_name", (q) =>
              q.eq("userId", args.userId).eq("name", ex.name)
            )
            .unique();
    ...
    return createdTemplateClientIds;
  },
});

/**
 * Creates a workout plan and its days from AI tool call output.
 */
export const createPlanFromAI = internalMutation({
  args: {
    userId: v.id("users"),
    plan: v.object({
      clientId: v.string(),
  ...
  handler: async (ctx, args) => {
    ...
    const existing = await ctx.db
      .query("workoutPlans")
      .withIndex("by_user_clientId", (q) =>
        q.eq("userId", args.userId).eq("clientId", args.plan.clientId)
      )
      .unique();
    ...
    const planId = await ctx.db.insert("workoutPlans", {
      ...
      goal: args.plan.goal,
      durationWeeks: args.plan.durationWeeks,
      startDate: args.plan.startDate,
      status: "active",
      sourceConversationClientId: args.plan.sourceConversationClientId,
      createdAt: now,
      updatedAt: now,
    });

    // Create all plan days
    for (const day of args.days) {
      await ctx.db.insert("planDays", {
        userId: args.userId,
        planClientId: args.plan.clientId,
        week: day.week,
...
/**
 * Creates a recipe from AI tool call output.
 */
export const createRecipeFromAI = internalMutation({
  args: {
    userId: v.id("users"),
    clientId: v.string(),
    title: v.string(),
