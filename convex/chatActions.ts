...
const AI_EXERCISE_ITEM = {
  ...
  properties: {
    ...
    metrics: {
      ...
      description:
        'Ordered metrics this exercise tracks (max 5), composed from the palette. Examples: strength ["weight","reps"]; bodyweight ["reps"]; running/rowing ["duration","distance","pace","heart_rate_avg"]; treadmill ["duration","distance","incline","pace"]; cycling ["duration","distance","speed","heart_rate_avg"]; watts bike ["duration","power_avg","distance","heart_rate_avg"]; timed hold ["duration"]. Required: must be non-empty for every non-interval exercise and match the movement (a run must NOT get ["weight","reps"]). For "intervals" exercises pass an empty array.',
    },
    type: {
      type: "string",
      enum: [
